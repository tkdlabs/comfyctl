...
func parseComfyInputs(inputMap map[string]any) (map[string]ComfyNodeInput, error) {
	...
	for k, v := range inputMap {
		...
		case bool:
			result[k] = ComfyNodeInput { Type: ComfyBoolInput, Bool: v.(bool) }
		case []any:
			nodeRef, idx, err := parseNodeRef(v.([]any))
			if err != nil {
				return result, err
			}
		...
		default:
			return result, errors.New(fmt.Sprintf("Unknown node input type: %s -> %T", k, v))
		}
	}
	return result, nil
}
...
func parseMetaTitle(metaMap map[string]any) (string, error) {
	...
	if found {
		...
		return result, errors.New(
			fmt.Sprintf("title not found in meta map: %v", metaMap))
	}
}
